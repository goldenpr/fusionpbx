...
//delete the user from the ring group
	if (
		...
		//redirect the browser
			header("Location: ring_group_edit2.php?id=$ring_group_uuid");
			exit;
	}
...
//assign the user to the ring group
	if (is_uuid($_REQUEST["user_uuid"]) && is_uuid($_REQUEST["id"]) && $_GET["a"] != "delete" && permission_exists("ring_group_edit")) {
		...
		//redirect the browser
			header("Location: ring_group_edit2.php?id=".urlencode($ring_group_uuid));
			exit;
	}
...
echo "	<tr>";
	...
	echo "					<td style='display:none' class='vtable' id='destination_delayorder'>";
	echo 						($ring_group_strategy == 'sequence' || $ring_group_strategy == 'rollover') ? $text['label-destination_order'] : $text['label-destination_delay'];
	echo "					</td>\n";
	echo "					<td style='display:none' class='vtable'>".$text['label-destination_timeout']."</td>\n";
	if (permission_exists('ring_group_prompt')) {
		echo "				<td style='display:none' class='vtable'>".$text['label-destination_prompt']."</td>\n";
	}
	echo "					<td></td>\n";
	echo "				</tr>\n";
	...
	foreach($ring_group_destinations as $row) {
		...
		echo "			<tr>\n";
		echo "				<td>\n";
		echo "					<select name='ring_group_destinations[".$x."][destination_number]' class='formfld' style='width: auto' value='".$row['destination_number']."'>\n";
		echo "						<option value='".$row['destination_number']."' ".(($row['destination_number'])?"selected='selected'":null).">".$row['destination_number']."</option>\n";
		echo "						<option value='Follow-Me-(אלי-מילנר)'>אלי מילנר</option>\n";
		echo "						<option value='Follow-Me-(דרור-אדיר)'>דרור אדיר</option>\n";
		echo "						<option value='Follow-Me-(נהלה-חורי)'>נהלה חורי</option>\n";
		echo "						<option value='Follow-Me-(שירלי-לוי)'>שירלי לוי</option>\n";
		echo "						<option value='Follow-Me-(שנית-ייגר)'>שנית ייגר</option>\n";
		echo "						<option value='Follow-Me-(אילנית-דורי)'>אילנית דורי</option>\n";
		echo "						<option value='Follow-Me-(אילנית-דורי)'>אילנית דורי</option>\n";
		echo "						<option value='Follow-Me-(אייל-בן-שמעון)'>אייל בן שמעון</option>\n";
		echo "						<option value='Follow-Me-(קובי-תורג'מן)'>קובי תורג'מן</option>\n";
		echo "						<option value='Follow-Me-(שמעון-ביטון)'>שמעון ביטון</option>\n";
		echo "						<option value='Follow-Me-(הנרי-כהן)'>הנרי כהן</option>\n";
		echo "						<option value='Follow-Me-(שלומי-אטיאס)'>שלומי אטיאס</option>\n";
		echo "						<option value='Follow-Me-(ליאור-בן-ארוש)'>ליאור בן ארוש</option>\n";
		echo "						<option value='Follow-Me-(ניצה-איפרגן)'>ניצה איפרגן</option>\n";
		echo "						<option value='Follow-Me-(כרמית-איבגי)'>כרמית איבגי</option>\n";
		echo "						<option value='Follow-Me-(מירב-אוזן)'>מירב אוזן</option>\n";
		echo "						<option value='Follow-Me-(ורד-דישון)'>ורד דישון</option>\n";
		//echo "						<option value='Follow-Me-(Shadid)'>Shadid</option>\n";
		echo "					</select>\n";
		echo "				</td>\n";
		/*echo "				<td>\n";
		...
		if (strlen($row['ring_group_destination_uuid']) > 0) {
			echo				"<a href='ring_group_destination_delete2.php?id=".$row['ring_group_destination_uuid']."&ring_group_uuid=".$row['ring_group_uuid']."&a=delete' alt='delete' onclick=\"return confirm('".$text['confirm-delete']."')\">$v_link_label_delete</a>";
		}
		echo "				</td>\n";
		echo "			</tr>\n";
		$x++;
	...
	if (is_array($ring_group_users) && @sizeof($ring_group_users) != 0) {
		...
		foreach ($ring_group_users as $field) {
			...
			echo "				<td>\n";
			echo "					<a href='ring_group_edit2.php?id=".urlencode($ring_group_uuid)."&user_uuid=".urlencode($field['user_uuid'])."&a=delete' alt='".$text['button-delete']."' onclick=\"return confirm('".$text['confirm-delete']."')\">".$v_link_label_delete."</a>\n";
			echo "				</td>\n";
			echo "			</tr>\n";
		}
		echo "		</table>\n";
