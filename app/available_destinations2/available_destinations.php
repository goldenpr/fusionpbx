...
//check permissions
	if (permission_exists('available_destinations_view')) {
		//access granted
	}
	else {
		echo "access denied";
...
//process the http post data by action
	if ($action != '' && is_array($available_destinations) && @sizeof($available_destinations) != 0) {
		switch ($action) {
			case 'copy':
				if (permission_exists('available_destinations_add')) {
...
//add the search string
	if (isset($_GET["search"])) {
		...
		$sql_search .= "	or lower(available_destinations_enabled) like :search ";
...
//get the count
	$sql = "select count(available_destinations_uuid) from v_available_destinations ";
	if ($_GET['show'] == "all" && permission_exists('available_destinations_all')) {
		if (isset($sql_search)) {
...
//prepare to page the results
	...
	$param = ($_GET['show'] == 'all' && permission_exists('available_destinations_all')) ? "&show=all" : null;
...
//get the list
	$sql = str_replace('count(available_destinations_uuid)', '*', $sql);
...
//show the content
	...
	if (permission_exists('available_destinations_add')) {
		echo button::create(['type'=>'button','label'=>$text['button-add'],'icon'=>$_SESSION['theme']['button_icon_add'],'id'=>'btn_add','link'=>'available_destinations_edit.php']);
	}
	if (permission_exists('available_destinations_add') && $available_destinations) {
		echo button::create(['type'=>'button','label'=>$text['button-copy'],'icon'=>$_SESSION['theme']['button_icon_copy'],'id'=>'btn_copy','name'=>'btn_copy','style'=>'display: none;','onclick'=>"modal_open('modal-copy','btn_copy');"]);
	}
	if (permission_exists('available_destinations_edit') && $available_destinations) {
		echo button::create(['type'=>'button','label'=>$text['button-toggle'],'icon'=>$_SESSION['theme']['button_icon_toggle'],'id'=>'btn_toggle','name'=>'btn_toggle','style'=>'display: none;','onclick'=>"modal_open('modal-toggle','btn_toggle');"]);
	}
	if (permission_exists('available_destinations_delete') && $available_destinations) {
		echo button::create(['type'=>'button','label'=>$text['button-delete'],'icon'=>$_SESSION['theme']['button_icon_delete'],'id'=>'btn_delete','name'=>'btn_delete','style'=>'display: none;','onclick'=>"modal_open('modal-delete','btn_delete');"]);
	}
	echo 		"<form id='form_search' class='inline' method='get'>\n";
	if (permission_exists('available_destinations_all')) {
		if ($_GET['show'] == 'all') {
	...
	if (permission_exists('available_destinations_add') || permission_exists('available_destinations_edit') || permission_exists('available_destinations_delete')) {
	...
	echo th_order_by('destination_trunk_name', $text['label-destination_trunk_name'], $order_by, $order);
	...
	if (permission_exists('available_destinations_edit') && $_SESSION['theme']['list_row_edit_button']['boolean'] == 'true') {
	...
	if (is_array($available_destinations) && @sizeof($available_destinations) != 0) {
		...
		foreach ($available_destinations as $row) {
			if (permission_exists('available_destinations_edit')) {
				$list_row_url = "available_destinations_edit.php?id=".urlencode($row['available_destinations_uuid']);
			}
			...
			if (permission_exists('available_destinations_add') || permission_exists('available_destinations_edit') || permission_exists('available_destinations_delete')) {
				...
				echo "		<input type='hidden' name='available_destinations[$x][uuid]' value='".escape($row['available_destinations_uuid'])."' />\n";
				echo "	</td>\n";
			}
			if ($_GET['show'] == 'all' && permission_exists('available_destinations_all')) {
